...
* Python source files:
  * The Python package source files are located in the `effectivesoftwaretesting` directory.
  * The file `effectivesoftwaretesting.__main__.py` defines a command line interface that
    is accessible both as the command `effectivesoftwaretesting` and via
    `python -m effectivesoftwaretesting`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_effectivesoftwaretesting.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `effectivesoftwaretesting`.
